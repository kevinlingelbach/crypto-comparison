...
To run the backend, navigate to /crypto-comparison/flask-server and switch into the the Python virtual environment using the command `python -m venv c:\path\to\windows-venv\scripts\activate.bat`.
To install the required Python libaries, navigate to /crypto-comparison using `pip install -r requirements.txt`
